...
import { makeTempRepo, makeTempRepoWithRemote } from "github-action-pack-test-toolkit";
...
describe("stageFilesAndCommitAndPush", () => {
  test("basic invocation", async () => {
    ...
    try {
      ...
      await fs.writeFile(path.join(repoPath, optionalFileName), optionalFileName);
      await fs.writeFile(path.join(repoPath, requiredFileName), requiredFileName);
      ...
      const remoteFinalLog = (await remoteGit.log({ from: initialRef, maxCount: 1 })).latest;
      if (remoteFinalLog === null) {
        throw Error("Failed to fetch log from remote repo");
      }
      console.log(remoteFinalLog);
