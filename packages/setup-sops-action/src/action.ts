// The content of this file is taken from https://raw.githubusercontent.com/mdgreenwald/mozilla-sops-action/main/src/run.ts
        ...
        responseArray.forEach(response => {
            if (response && response.tag_name) {
                ...
                if (currentSopsVerison) {
                    if (
                      currentSopsVerison.toString().indexOf('rc') == -1 &&
                      semver.gt(currentSopsVerison, latestSopsVersion)
                    ) {
...
async function run() {
    ...
    try {
        ...
        if (pathVar && !pathVar.startsWith(path.dirname(cachedPath))) {
